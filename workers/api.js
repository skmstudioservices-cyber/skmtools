// SKMTools Workers API — All routes
...
export default {
  async fetch(request, env) {
    ...
    if (path.startsWith('/api/admin')) {
      ...
      // ADMIN: CREATE/UPDATE BLOG POST (superadmin only)
      if (path === '/api/admin/blog' && request.method === 'POST') {
        if (admin.role !== 'superadmin') return json({ error: 'Forbidden' }, 403);
        const body = await request.json();
        const { slug, title, meta_description, category, country_target, content, published } = body;
        await env.DB.prepare(
          'INSERT OR REPLACE INTO blog_posts (slug, title, meta_description, category, country_target, content, published) VALUES (?,?,?,?,?,?,?)'
        ).bind(slug, title, meta_description, category, country_target, content, published ? 1 : 0).run();
        return json({ success: true });
      }

      // ADMIN: UPDATE LEAD TIER (superadmin only)
      if (path === '/api/admin/update-tier' && request.method === 'POST') {
        if (admin.role !== 'superadmin') return json({ error: 'Forbidden' }, 403);
        const body = await request.json();
        await env.DB.prepare('UPDATE leads SET tier = ?, plan = ? WHERE email = ?').bind(body.tier, body.plan, body.email).run();
        return json({ success: true });
      }
    }

    // ── PRICES (country aware) ──
    if (path === '/api/prices') {
      const country = (request.headers.get('CF-IPCountry') || 'US').toLowerCase();
      const prices = {
    ...
    return new Response('SKMTools API — Route not found', { status: 404 });
  }
};
